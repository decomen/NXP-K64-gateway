...
static void w25qxx_wait_busy_erase(void)
{
...
static uint32_t w25qxx_read(uint32_t offset, uint8_t * buffer, uint32_t size)
{
    ...
    send_buffer[send_index++] = CMD_READ;
    if( spi_flash_device.geometry.sector_count > 4096 ) {
        send_buffer[send_index++] = (uint8_t)(offset>>24);
    }
    send_buffer[send_index++] = (uint8_t)(offset>>16);
    send_buffer[send_index++] = (uint8_t)(offset>>8);
    send_buffer[send_index++] = (uint8_t)(offset);
...
uint32_t w25qxx_page_write(uint32_t page_addr, const uint8_t* buffer)
{
    ...
    send_buffer[send_index++] = CMD_ERASE_4K;
    if( spi_flash_device.geometry.sector_count > 4096 ) {
        send_buffer[send_index++] = (uint8_t)(page_addr>>24);
    }
    send_buffer[send_index++] = (page_addr >> 16);
    send_buffer[send_index++] = (page_addr >> 8);
    send_buffer[send_index++] = (page_addr);
    rt_spi_send(spi_flash_device.rt_spi_device, send_buffer, send_index);

    w25qxx_wait_busy_erase(); // wait erase done.
    ...
    for(index=0; index < (PAGE_SIZE / 256); index++)
    {
        ...
        send_index = 0;
        send_buffer[send_index++] = CMD_PP;
        if( spi_flash_device.geometry.sector_count > 4096 ) {
            send_buffer[send_index++] = (uint8_t)(page_addr>>24);
        }
        send_buffer[send_index++] = (uint8_t)(page_addr >> 16);
        send_buffer[send_index++] = (uint8_t)(page_addr >> 8);
        send_buffer[send_index++] = (uint8_t)(page_addr);

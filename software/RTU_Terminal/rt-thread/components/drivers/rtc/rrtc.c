...
rt_err_t set_timestamp( rt_uint32_t timestamp )
{
    rt_device_t device;
    rt_err_t ret = -RT_ERROR;
    ...
    /* update to RTC device. */
    ret = rt_device_control(device, RT_DEVICE_CTRL_RTC_SET_TIME, &timestamp);
...
FINSH_FUNCTION_EXPORT(set_timestamp, set time. e.g: set_timestamp(1477752060))
#endif

...
static rt_err_t alarm_set(struct rt_alarm *alarm)
{
    ...
    if (alarm->flag & RT_ALARM_STATE_START) wkalarm.enable = RT_TRUE;
    else wkalarm.enable = RT_FALSE;
...
static void alarm_wakeup(struct rt_alarm *alarm, struct tm *now)
{
    ...
    if (alarm->flag & RT_ALARM_STATE_START) {
        ...
        case RT_ALARM_DAILY:
        {
            if (((sec_now - sec_alarm) <= RT_ALARM_DELAY) && (sec_now >= sec_alarm)) wakeup = RT_TRUE;
        }
            break;
        ...
        case RT_ALARM_MONTHLY:
        {
            ...
            if (alarm->wktime.tm_mday == now->tm_mday) {
                if ((sec_now - sec_alarm) <= RT_ALARM_DELAY) wakeup = RT_TRUE;
            }
        }
            break;
...
static void alarm_update(rt_uint32_t event)
{
    ...
    if (!rt_list_isempty(&_container.head)) {
        ...
        if (sec_next < 24 * 3600) {
            if (alarm_set(alm_next) == RT_EOK) _container.current = alm_next;
        } else if (sec_prev < 0) {
            /* enable the alarm before now */
            if (alarm_set(alm_prev) == RT_EOK) _container.current = alm_prev;
        }
    }
...
static rt_err_t alarm_setup(rt_alarm_t alarm, struct tm *wktime)
{
    ...
    *setup = *wktime;
    ...
    if ((setup->tm_sec > 59) || (setup->tm_sec < 0)) setup->tm_sec = now.tm_sec;
    if ((setup->tm_min > 59) || (setup->tm_min < 0)) setup->tm_min = now.tm_min;
    if ((setup->tm_hour > 23) || (setup->tm_hour < 0)) setup->tm_hour = now.tm_hour;
    ...
    case RT_ALARM_ONESHOT:
    {
        ...
        if (setup->tm_year == RT_ALARM_TM_NOW) setup->tm_year = now.tm_year;
        if (setup->tm_mon == RT_ALARM_TM_NOW) setup->tm_mon = now.tm_mon;
        if (setup->tm_mday == RT_ALARM_TM_NOW) setup->tm_mday = now.tm_mday;
        /* make sure the setup is valid */
        if (!is_valid_date(setup)) goto _exit;
    }
        break;
    ...
    case RT_ALARM_WEEKLY:
    {
        ...
        if ((setup->tm_wday < 0) || (setup->tm_wday > 6)) setup->tm_wday = now.tm_wday;
    }
        break;
    ...
    case RT_ALARM_MONTHLY:
    {
        ...
        if ((setup->tm_mday < 1) || (setup->tm_mday > 31)) setup->tm_mday = now.tm_mday;
    }
        break;
    ...
    case RT_ALARM_YAERLY:
    {
        ...
        if ((setup->tm_mon < 0) || (setup->tm_mon > 11)) setup->tm_mon = now.tm_mon;

        if (setup->tm_mon == 1) {
            /* tm_mon is February */

            /* tm_mday should be 1~29.otherwise,it's a "don't care" value */
            if ((setup->tm_mday < 1) || (setup->tm_mday > 29)) setup->tm_mday = now.tm_mday;
            ...
            if ((setup->tm_mday < 1) || (setup->tm_mday > 30)) setup->tm_mday = now.tm_mday;
        }
    }
        break;
...
rt_err_t rt_alarm_start(rt_alarm_t alarm)
{
    ...
    if (alarm == RT_NULL) return ret;
    rt_mutex_take(&_container.mutex, RT_WAITING_FOREVER);
    if (!(alarm->flag & RT_ALARM_STATE_INITED)) {
        if (alarm_setup(alarm, &alarm->wktime) != RT_EOK) goto _exit;
    }
...
rt_err_t rt_alarm_stop(rt_alarm_t alarm)
{
    ...
    if (alarm == RT_NULL) return ret;
    rt_mutex_take(&_container.mutex, RT_WAITING_FOREVER);
    if (!(alarm->flag & RT_ALARM_STATE_START)) goto _exit;
    /* stop alarm */
    alarm->flag &= ~RT_ALARM_STATE_START;
...
rt_alarm_t rt_alarm_create(rt_alarm_callback_t callback, struct rt_alarm_setup *setup)
{
    ...
    if (setup == RT_NULL) return RT_NULL;
    alarm = rt_malloc(sizeof(struct rt_alarm));
    if (alarm == RT_NULL) return RT_NULL;

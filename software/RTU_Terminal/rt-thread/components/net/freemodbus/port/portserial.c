...
#define EVENT_TRANS_START_ANY           (~(0xFFFFFFFFUL << BOARD_UART_MAX))

#define MB_THREAD_TRAN_NAME             "mbs_tr"                             
#define MB_EVENT_NAME                   "mbs_tre"
        ...
        eMBParity eParity)
{
    ...
    /* open serial device */
    if (RT_EOK == g_serials[ucPort]->parent.open(&g_serials[ucPort]->parent,
...
/**
 * Software simulation g_serials transmit IRQ handler.
 *
...
static void serial_trans_irq(void* parameter) {
    ...
    while (1)
    {
        ...
        for( UCHAR ucPort = 0; ucPort < BOARD_UART_MAX; ucPort++ ) {

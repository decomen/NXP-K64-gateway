...
void webnet_session_printf(struct webnet_session *session, const char *fmt, ...)
{
    ...
    rt_vsnprintf((char *)(session->buffer), session->buffer_length, fmt, args);
    va_end(args);

    lwip_send(session->socket, session->buffer, rt_strlen((char *)(session->buffer)), 0);
}
...
{
    /* send data directly */
    lwip_send(session->socket, data, size, 0);
...
void webnet_sessions_handle_fds(fd_set *readset, fd_set *writeset)
{
    ...
    read_buffer = (char *)WEBNET_MALLOC(WEBNET_SESSION_BUFSZ * 2);

...
int webnet_module_system_do_uploadfile(struct webnet_session *session, const char *path, rt_bool_t reset);
int webnet_module_system_do_uploadtext(struct webnet_session *session, const char *path, rt_bool_t reset);
...
static int _webnet_module_system_uri_physical(struct webnet_session *session, int event)
{
    ...
    if (str_begin_with(session->request->path, "/file/update0")) {
        result = webnet_module_system_do_uploadfile(session, "/update.img", RT_TRUE);
        if (result == WEBNET_MODULE_FINISHED) return result;
    } else if (str_begin_with(session->request->path, "/file/cfgrecover0")) {
        result = webnet_module_system_do_uploadfile(session, "/cfg/board_v0.cfg", RT_TRUE);
        if (result == WEBNET_MODULE_FINISHED) return result;
    } else if (str_begin_with(session->request->path, "/file/cfgrecover_with_json")) {
        result = webnet_module_system_do_uploadfile(session, "/cfg/board_json.cfg", RT_FALSE);
        if(200 == session->request->result_code) {
            if(cfg_recover_with_json("/cfg/board_json.cfg")) {
                vDoSystemReset();
            }
        }
        if (result == WEBNET_MODULE_FINISHED) return result;
        ...
        if (0 == mkdir_p(filepath, 0755)) {
            result = webnet_module_system_do_uploadfile(session, filepath, RT_FALSE);
        }
        if (result == WEBNET_MODULE_FINISHED) return result;
    } else if (str_begin_with(session->request->path, "/ini/upload/")) {
        char *filepath = &session->request->path[strlen("/ini/upload/")];
        if (0 == mkdir_p(filepath, 0755)) {
            result = webnet_module_system_do_uploadtext(session, filepath, RT_FALSE);
        }
        if (result == WEBNET_MODULE_FINISHED) return result;
...
int webnet_module_system_do_uploadtext(struct webnet_session *session, const char *path, rt_bool_t reset)
{
    ...
    while (remain > 0) {
        ...
        if (read_bytes > 0) {
            write(fd, session->buffer, read_bytes);
            remain -= read_bytes;
        } else {
            break;
        }
...
int webnet_module_handle_uri(struct webnet_session *session)
{
    ...
    full_path = (char *)WEBNET_CALLOC(1,WEBNET_PATH_MAX);

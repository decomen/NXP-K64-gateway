...
{
  _mxml_fdbuf_t	*buf = (_mxml_fdbuf_t	*)rt_malloc(sizeof(_mxml_fdbuf_t));			/* File descriptor buffer */
  mxml_node_t *p = NULL;
  ...
  */
  ...
  p = (mxml_load_data(top, buf, cb, mxml_fd_getc, MXML_NO_CALLBACK, NULL));
  rt_free(buf);
  return p;
}
 ...
 * using the rt_free() function when you are done with it.  NULL is returned
...
#define _BUF_SZ (8192)
  ...
  char	*buffer = rt_malloc(_BUF_SZ);			/* Temporary buffer */
  ...
  */
  ...
  bytes = mxmlSaveString(node, buffer, _BUF_SZ, cb);
              ...
              void           *sax_data)	/* I - SAX user data */
{
  ...
  */
  ...
  p = (mxml_load_data(top, buf, cb, mxml_fd_getc, sax_cb, sax_data));
  rt_free(buf);
  return p;
}
 ...
 *
 * The save function accepts a node pointer and must return a rt_malloc'd
	      ...
	      int  *bufsize)		/* IO - Current buffer size */
{
  ...
  if (*bufptr >= (*buffer + *bufsize - 4))
  {
    ...
    */
    ...
    if ((newbuffer = rt_realloc(*buffer, *bufsize)) == NULL)
    {
      rt_free(*buffer);
...
#ifdef EINTR
    if (errno != DFS_STATUS_EAGAIN && errno != EINTR)
#else
    if (errno != DFS_STATUS_EAGAIN)
#endif /* EINTR */
      return (-1);
	...
	*/
	...
	else if ((ch & 0xe0) == 0xc0)
	{
	  ...
	  */
	  ...
	  if ((temp = fgetc(fp)) == EOF || (temp & 0xc0) != 0x80)
	    return (EOF);
...
#define _NODE_SZ    (255)
  ...
  char		*s = rt_malloc(_NODE_SZ);			/* Temporary string */

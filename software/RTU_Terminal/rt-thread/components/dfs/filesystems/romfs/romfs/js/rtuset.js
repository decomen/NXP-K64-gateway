    ...
    success: function(data){
        ...
        if(data.ret != null ) {
			if( data.ret == 401 ) {
				...
				window.location.href="http://"+window.location.hostname+"/reg.html";
			}
        }
...
function refreshAllVarManageExtDataBase(index)
{
    ...
    for( var n = 0; n < xVarManageExtDataBase.length; n++ ) {
        varExtTableAddItem( 
            ...
            varExtGetVarTypeName(xVarManageExtDataBase[n].vt,xVarManageExtDataBase[n].vs), 
            varExtGetProtoName(xVarManageExtDataBase[n].dt, xVarManageExtDataBase[n].dtn, xVarManageExtDataBase[n].pt), 
            xVarManageExtDataBase[n].sa, 
            xVarManageExtDataBase[n].ea, 
            xVarManageExtDataBase[n].ao, 
            xVarManageExtDataBase[n].se, 
            xVarManageExtDataBase[n].ss
        );
    }
...
var extdata_empty_val = {
	en:0, na: "++ADD++", al: "++ADD++", vt:0, vs:0, 
	dt:0, dtn:0, pt:0, ad:2048, 
	sa:0, ea:0, ao:0, rw:0,	it:1000, se:0, ss:5
};
            ...
            if( xhr.readyState==4 ) {
                ...
                if( xhr.status==200 ) {
                    ...
                    if( res && 0 == res.ret ) {
						...
						xVarManageExtDataBase.push(extdata_empty_val);
...
function onExtTableItemClick(tb,row)
{
    if( row.rowIndex >= 0 ) {
        ...
        setValue('var_ext_devtype', xVarManageExtDataBase[row.rowIndex].dt + "|" + xVarManageExtDataBase[row.rowIndex].dtn + "|" + xVarManageExtDataBase[row.rowIndex].pt);
        ...
        setValue('var_ext_storage_en', xVarManageExtDataBase[row.rowIndex].se);//数据存盘
        setValue('var_ext_storage_step', xVarManageExtDataBase[row.rowIndex].ss);//存盘间隔
...
function varExtGetProtoName(dev,n,proto)
{
    ...
    case 0: {
        ...
        case 1: return "UART1_Modbus_ASCII";
        }
        break;
    }
    case 1: {
        ...
        case 1: return "UART2_Modbus_ASCII";
        }
        break;
    }
    case 2: {
        ...
        case 1: return "UART3_Modbus_ASCII";
        }
        break;
    }
    case 3: {
        ...
        case 1: return "UART4_Modbus_ASCII";
        }
        break;
    }
    case 4: {
        switch(proto) {
        case 0: return "Net_Modbus_TCP_" + (n+1);
        case 1: return "北京大型公建通信协议_" + (n+1);
        }
        break;
    }
    case 5: {
        ...
        case 1: return "Zigbee_Modbus_ASCII";
        }
        break;
    }
    case 6: {
        switch(proto) {
        case 0: return "GPRS_Modbus_TCP_" + (n+1);
        }
        break;
    }
    }
...
function setVarExtInfo()
{
    ...
    if( id >= 0 && id <= 64 ) {
        ...
        xVarManageExtDataBase[id].dtn=setval.dtn;
...
function refreshProtoDevList(res)
{
    ...
    if( res.net != null ) {
        for( var n = 0; n < res.net.length; n++ ) {
            protoDevList.options.add(
                new Option(
                    varExtGetProtoName( res.net[n].id, res.rs[n].idn, res.net[n].po ), 
    ...
    if( res.zigbee != null ) {
        for( var n = 0; n < res.zigbee.length; n++ ) {
            protoDevList.options.add(
                new Option(
                    varExtGetProtoName( res.zigbee[n].id, res.rs[n].idn, res.zigbee[n].po), 
    ...
    if( res.gprs != null ) {
        for( var n = 0; n < res.gprs.length; n++ ) {
            protoDevList.options.add(
                new Option(
                    varExtGetProtoName( res.gprs[n].id, res.rs[n].idn, res.gprs[n].po), 
...
function Page_System_Resource(info)
{
    ...
    //内存剩余
    document.getElementById("sys_memory_free").innerHTML = "Free : " + (Number(info.ms)-Number(info.mu)) + " B  [Total : " + Number(info.ms) + " B, Max Used : " +  Number(info.ma) + " B]";
...
{
    //*注：在此处编写获取代码；然而调用如下函数，实现页面显示；
	MyGetJSON("","/cgi-bin/getZigbeeList?","", function (info)
    {
...
function zgbNodeTableAddItem(mode,mac,netid,on,rssi,adlst,upt,offt)
{
	...
	obj = row.insertCell(4);
	if(Number(on)!=0 && rssi!=null) {
		obj.innerHTML = (Number(rssi)+"dB");
	} else {
		obj.innerHTML = "未知";
	}
	obj = row.insertCell(5);
...
function Page_Time_Synchronization(cfg)
{
	...
	if( cfg.dbg != null ) {
		document.getElementsByName("base_debug")[0].checked = (Number(cfg.dbg)!=0);
		document.getElementsByName("base_debug")[1].checked = !(Number(cfg.dbg)!=0);
	} else {
		document.getElementsByName("base_debug")[0].checked = false;
		document.getElementsByName("base_debug")[1].checked = false;
	}
}
...
function Apply_Time_Synchronization()
{
    ...
    var base_debug_yes = document.getElementsByName("base_debug")[0].checked;
    var base_debug_no = document.getElementsByName("base_debug")[1].checked;
    ...
    if (chk2(base_debug_yes, base_debug_no, "调试串口")) return;
...
function Show_Remote_Transmission()
{
    ...
    MyGetJSON("正在获取以太网 TCP/IP配置","/cgi-bin/getTcpipCfg?","arg="+JSON.stringify(setval),function( info ){
		...
		setDisplay('remote_proto_ms', getValue('remote_proto')!=1);
		setDisplay('btn_remote_proto_ini', getValue('remote_proto')==1);
		setDisplay('cc_bjdc_ini', getValue('remote_proto')==1);
		load_remote_ini();
    });
}
...
function Show_ZIGBEE()
{
    ...
    MyGetJSON("正在获取Zigbee配置","/cgi-bin/getZigbeeCfg?", "", function( cfg ){
...
function Page_ZIGBEE(cfg)
{
    ...
    //发送模式
    setValue('ZIGBEE_Transmission_Mode', cfg.mmd);
}
...
function Apply_ZIGBEE()
{
    ...
    //发送模式
    var ZIGBEE_Transmission_Mode = getValue("ZIGBEE_Transmission_Mode");
    ...
    if (chk(ZIGBEE_Transmission_Mode, "发送模式")) return;
    ...
    var setval = {
        ...
        mmd:Number(ZIGBEE_Transmission_Mode)
    };
    //注：在此处填写保存代码即可；
    MyGetJSON("正在配置Zigbee,请稍后...","/cgi-bin/setZigbeeCfg?","arg=" + JSON.stringify(setval));
}
...
function Apply_Output()
{
    ...
    //注：在此处填写保存代码即可；
    MyGetJSON("正在设置输出模块信息","/cgi-bin/setDoCfg?","arg=" + JSON.stringify(onputset));
}
...
function Show_Script()
{
    ...
    s+='<tr style="background-color:rgb(6,96,255)"><td style="width:80%">脚本标题</td><td style="width:10%">脚本时间</td><td style="width:10%">操作</td></tr>';
    //------------重复-----------------
...
var xfer_net_cfgs = new Array();

function Xfer_Net_Change()
{
    //是否启用 以太网
    {
        var xfer_net_radio_0=$("input[name='xfer_net_radio_0']:checked").val();
        
        if(xfer_net_radio_0=="YES"){
            $('#form_enet input,#form_enet select,#form_enet').attr("disabled",false);
        }else{
            $('#form_enet input,#form_enet select,#form_enet').attr("disabled",true);
        }
        
        setEnable('xfer_net_dst_uart_type_0',false);
    }
    //是否启用 GPRS转发
    {
        var xfer_net_radio_1=$("input[name='xfer_net_radio_1']:checked").val();
        
        if(xfer_net_radio_1=="YES"){
            $('#form_gprs input,#form_gprs select,#form_gprs').attr("disabled",false);
        }else{
            $('#form_gprs input,#form_gprs select,#form_gprs').attr("disabled",true);
        }
        
        setEnable('xfer_net_dst_uart_type_1',false);
    }
	
	{
        //转发接口 调用串口模式
        // 当前转发接口val
        var xfer_net_dst_type_0=getNumber("xfer_net_dst_type_0");
		if(Number(xfer_net_dst_type_0)>=0 && Number(xfer_net_dst_type_0) <= 4) {
			setValue('xfer_net_dst_uart_type_0', '-1');
			for( var n = 0; n < xfer_net_cfgs.length; n++ ) {
				if( xfer_net_cfgs[n].dt == Number(xfer_net_dst_type_0) ) {
					setValue('xfer_net_dst_uart_type_0', xfer_net_cfgs[n].uty);
				}
			}
			$('.lable_none').css('display','block');
		} else {
			$('.lable_none').css('display','none');
		}
     }
	 
     {
        //GPRS转发  转发接口 调用串口模式
        // 当前转发接口val
        var xfer_net_dst_type_1=getNumber("xfer_net_dst_type_1");
		if(Number(xfer_net_dst_type_1)>=0 && Number(xfer_net_dst_type_1) <= 4) {
			setValue('xfer_net_dst_type_1', '-1');
			for( var n = 0; n < xfer_net_cfgs.length; n++ ) {
				if( xfer_net_cfgs[n].dt == Number(xfer_net_dst_type_1) ) {
					setValue('xfer_net_dst_type_1', xfer_net_cfgs[n].uty);
				}
			}
			$('.lable_clear_none').css('display','block');
		} else {
			$('.lable_clear_none').css('display','none');
		}
     }
}

//转发
function Show_Xfer_Net_Cfg()
{
    MyGetJSON("正在获取转发配置","/cgi-bin/getXferNetCfg?", "arg={\"all\":1}",function( cfg ){
		if( cfg != null && cfg.ret == 0 && cfg.list != null ) {
			xfer_net_cfgs = cfg.list.concat();
			for( var n = 0; n < cfg.list.length; n++ ) {
				Page_Xfer_Net_Cfg(n,cfg.list[n]);
			}
			Xfer_Net_Change();
		}
    });
...
function Page_Xfer_Net_Cfg(n,cfg)
{
    ...
    if( cfg.udb != null && cfg.usb != null ) setValue('xfer_net_dst_uart_bits_'+n, Number(cfg.udb * 10) + Number(cfg.usb));
}
...
function setXferNetCfg(n)
{
    ...
    var setval = {
        ...
        udb:parseInt(Number(xfer_net_dst_uart_bits_)) / 10, 
        usb:parseInt(Number(xfer_net_dst_uart_bits_)) % 10
    };    
    
    MyGetJSON("正在设置转发配置","/cgi-bin/setXferNetCfg?","arg=" + JSON.stringify(setval));
}
...
function load_remote_ini()
{
	if(getValue('remote_proto')==1) {
		$.ajax({
			type: "get",
			url: "/download/cfg/cc_bjdc_"+getValue('remote_group')+".ini",
			dataType: "html",
			data: "",
			cache: false,
			success: function(data){
				 $("#cc_bjdc_ini_content").val(data);
			}
		}); 
	}
}

function save_remote_ini()
{
    var _val = $("#cc_bjdc_ini_content").val();
     $.ajax({
        type: "post",
        url: "/ini/upload/cfg/cc_bjdc_"+getValue('remote_group')+".ini",
        data:_val,
        dataType: "html",
        cache: false,
        success: function(data){

...
rt_time_t rt_millisecond_from_tick(rt_uint32_t tk)
{
#if !(1000 % RT_TICK_PER_SECOND)
    return (1000 / RT_TICK_PER_SECOND) * tk;
#else
    if( tk < (0xFFFFFFFF / 1000) ) {
        return (1000 * tk) / RT_TICK_PER_SECOND;
    } else {
        return (tk / RT_TICK_PER_SECOND) * 1000;
    }
#endif
}

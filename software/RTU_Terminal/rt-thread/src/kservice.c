...
/**
 * This function close the device using in console.
 *
 * @return the device using in console or RT_NULL
 */
 ...
 *  
 * @param in 源字符串
 * @param out 存放最后结果的字符串
 * @param outlen out最大的大小
 * @param src 要替换的字符串
 * @param dst 替换成什么字符串 
 *  
 * @return if err NULL, else return out
 */
char* rt_strrpl(char *in, char *out, int outlen, const char *src, char *dst) {
    char *p = in;
    unsigned int  len = outlen - 1;

    // 这几段检查参数合法性
    if ((RT_NULL == src) || (RT_NULL == dst) || (RT_NULL == in) || (RT_NULL == out)) {
        return RT_NULL;
    }
    if (in[0] == '\0' || src[0] == '\0') {
        return RT_NULL;
    ...
    while ((*p != '\0') && (len > 0)) {
        if (strncmp(p, src, strlen(src)) != 0) {
            ...
            if (strlen(out) + strlen(dst) > outlen) {
                return RT_NULL;
            }
            strcat(out, dst);
            p += strlen(src);

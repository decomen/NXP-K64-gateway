...
#define REG_INFO_OFS            (1024*1024-2*4096)
#define REG_INFO_SIZE           (4096)
...
{
    if( !board_cfg_read( REG_CFG_OFS, &g_reg, sizeof(g_reg) ) ) {
        prvSetRegDefault();
    }
}
...
{
    if( !board_cfg_write( REG_CFG_OFS, &g_reg, sizeof(g_reg) ) ) {
        prvSetRegDefault();
    }
}
...
static void prvReadRegInfoFromRom( void )
{
    ...
    memcpy( &magic, (void const *)REG_INFO_OFS, 4 );
    if( CFG_MAGIC == magic ) {
        memcpy( &g_reg_info, (void const *)(REG_INFO_OFS+4), sizeof(g_reg_info) );
        memcpy( &usCheck, (void const *)(REG_INFO_OFS+4+sizeof(g_reg_info)), 2 );
        if( usMBCRC16( &g_reg_info, sizeof(g_reg_info) ) != usCheck ) {
...
static void prvSaveRegInfoToRom( void )
{
    ...
    if( buffer ) {
        ...
        usCheck = usMBCRC16( &g_reg_info, sizeof(g_reg_info) );
        memcpy( buffer + 4 + sizeof(g_reg_info), &usCheck, 2 );

        FLASH_EraseSector( REG_INFO_OFS );
        FLASH_WriteSector( REG_INFO_OFS, buffer, 4096 );
...
{
    rt_uint32_t in[4] = { SIM->UIDH, SIM->UIDMH, SIM->UIDML, SIM->UIDL };
    //char *phwid = (char *)in;
    ...
    sprintf( buf, "%02X%08X%02X%08X%02X%08X%02X%08X", code&0xFF,~in[0],(code>>8)&0xFF,~in[1],(code>>16)&0xFF,~in[2],(code>>24)&0xFF,~in[3] );
...
rt_bool_t reg_check( const char *key )
{
    if( key[0] ) {
        ...
        {
            char deskey[] = { 
                g_regid[1]+~0x5F, ~g_regid[9]+0x2D, ~g_regid[10]+0x25, g_regid[16]+~0x34, 
                g_regid[5]-~0xAD, g_regid[2]-0x58, ~g_regid[22]+~0xEB, ~g_regid[7]-~0xA0 
            };
            rtu_des_encrypt( mykey, (unsigned char *)mykey, REG_ID_LEN, deskey );
...
rt_bool_t reg_hclient_query(void)
{
    ...
    if (session) {
        ...
        if (json) {
            ...
            rt_sprintf(strbuf, "%u", g_reg_info.code);

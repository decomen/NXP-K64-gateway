...
void vTestGetAdc(const S_MSG *msg, byte_buffer_t *bb)
{
	...
	int cnt = 0;
	for(int i = 0; i < 10;i++){
		vGetAdcValueTest( request.Type, Range_0_20MA, Range_0_20MA, 0, 100, factor, &xAdcVal); //用于校准的值必须求平均值
		if(xAdcVal.usEngUnit != 0){
			EngUnit += xAdcVal.usEngUnit;
			Measure += xAdcVal.fMeasure;
			rt_thread_delay( CHANNEL_CHANGE_TIME * 10 / 1000 );
			cnt++;
		}
	}
...
void vTestSetAdc(const S_MSG *msg, byte_buffer_t *bb)
{
	...
	if(request.xAdcInfo.usRange == CHECK_VAL_20_MA && chann == ADC_CHANNEL_7){
		...
		for(int i = 0 ; i < 8 ;i++){
			if((gCalEntryBak[i].xMin.usAdcValue <= (209715 - 30000)) || (gCalEntryBak[i].xMin.usAdcValue >= (209715 + 30000))){
				set_ok = mdFALSE;
				break;
			}
			if((gCalEntryBak[i].xMiddle.usAdcValue <= (524287 - 30000)) || (gCalEntryBak[i].xMiddle.usAdcValue >= (524287 + 30000))){
				set_ok = mdFALSE;
				break;
			}
			if((gCalEntryBak[i].xMax.usAdcValue <= (943717 - 30000)) || (gCalEntryBak[i].xMax.usAdcValue >= (943717 + 30000))){
				set_ok = mdFALSE;
			}
		}
		if(set_ok != mdTRUE){

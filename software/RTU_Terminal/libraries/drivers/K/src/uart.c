 ...
 * \retval None
 */
void UART_putstr(uint32_t instance, const char *str)
{
    while(*str != '\0')
    {
...
static void UART_IRQ_Handler(uint32_t instance)
{
    ...
    if(UARTBase[instance]->S1 & UART_S1_OR_MASK)
    {
	 ...
	 if(UARTBase[instance]->S1 & UART_S1_PF_MASK)
    {
        dummy = UARTBase[instance]->D;
    }

	  if(UARTBase[instance]->S1 & UART_S1_FE_MASK)
    {
        dummy = UARTBase[instance]->D;
    }

	if(UARTBase[instance]->S1 & UART_S1_NF_MASK)
    {
        dummy = UARTBase[instance]->D;
    }

...
rt_bool_t cc_bjdc_insert_item(rt_uint8_t index, double value)
{
    if (s_nCC_BJDC_DescId[index] < 0) {
        const ini_t *ini = s_xCC_BJDC_cfg[index];
        if (ini) {
            const char *meter_id = ini_getstring(ini, "relate_0", "meter_id", RT_NULL);
            if (RT_NULL == meter_id || '\0' == meter_id[0]) return RT_FALSE;
            const char *func_idex = ini_getstring(ini, "relate_0", "func_idex", RT_NULL);
            if (RT_NULL == func_idex || '\0' == func_idex[0]) return RT_FALSE;
            s_nCC_BJDC_DescId[index] = nStorage_CC_BJDC_DataDesc(meter_id, func_idex);
        }
    }

    if (s_nCC_BJDC_DescId[index] >= 0) {
        return (bStorageAdd_CC_BJDC_Data(s_nCC_BJDC_DescId[index], value));
    }
...
void cc_bjdc_exitwork(rt_uint8_t index)
{
    // do nothing
    /*if (s_bjdc_work_thread[index]) {
        if (RT_EOK == rt_thread_delete(s_bjdc_work_thread[index])) {
...
{
    {
        ...
        for (int n = 0; n < usBytesLen; n++) {
            ...
            if (CC_BJDC_R_S_HEAD == eRcvState) {
                if (4 == nPos) {
                    if (CC_BJDC_PRE == *(rt_uint32_t *)pBuffer) {
                ...
                if (nPos >= 8) {
                    if (CC_BJDC_EOM == *(rt_uint32_t *)&pBuffer[nPos - 4] && nPos >= 18) {
...
_SATRT:
    if (ini) {
        ...
        rt_kprintf("...verify ok !\n");
        {
            ...
            char szSW[8] = { 0 };
            char buf[64] = { 0 };
            memcpy(szSN, g_xDevInfoReg.xDevInfo.xSN.szSN, sizeof(DevSN_t));
            sprintf(szHW, "%d.%02d", HW_VER_VERCODE / 100, HW_VER_VERCODE % 100);
            sprintf(szSW, "%d.%02d", SW_VER_MAJOR, SW_VER_MINOR);
        ...
        while (1) {
            ...
            if (RT_EOK == rt_mq_recv(s_CCDataQueue[index], &xData, sizeof(CC_BJDC_Data_t), 2 * RT_TICK_PER_SECOND)) {
                ...
                case CC_BJDC_PT_DATA_QUERY:
                {
                    ...
                    rt_enter_critical();
                    {
                        ...
                        while (1) {
                            ...
                            if (node) {
                                var_double_t ext_value = 0;
                                if (node->xUp.bEnable) {
                                    cc_bjdc_data_add_data(xml, EXT_DATA_GET_NID(node->xUp.szNid), node->xUp.szFid, node->xUp.xAvgUp.val_cur);
                                }
                            } else {
                                break;
                            }
                    ...
                    cc_bjdc_send_xml(index, xml, 2048);
                    break;
                }
                }
            }
                                ...
                                if (node->xUp.bEnable) {
                                    if (up_interval > 0 && rt_tick_get() - node->xUp.xAvgUp.time >= rt_tick_from_millisecond(up_interval * 1000)) {
                                        if (node->xUp.xAvgUp.count > 0) {
                                            ...
                                            cc_bjdc_data_add_data(xml, EXT_DATA_GET_NID(node->xUp.szNid), node->xUp.szFid, value);
...
#define _CC_BUF_SZ     (1024)
    ...
    if (netif_list) {
        ...
        mxmlNewText(mxmlNewElement(device, "mac"), 0, buf);
        mxmlNewText(mxmlNewElement(device, "ip"), 0, ipaddr_ntoa(&(netif_list->ip_addr)));
        mxmlNewText(mxmlNewElement(device, "mask"), 0, ipaddr_ntoa(&(netif_list->netmask)));
        mxmlNewText(mxmlNewElement(device, "gate"), 0, ipaddr_ntoa(&(netif_list->gw)));
    }

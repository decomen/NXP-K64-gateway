...
static void prvSetHostCfgDefault( void )
{
    ...
    rt_sprintf( g_host_cfg.szHostName, "%s_%02X%02X",
        g_host_cfg.szHostName,
        g_xDevInfoReg.xDevInfo.xNetMac.mac[4],
        g_xDevInfoReg.xDevInfo.xNetMac.mac[5]
    );
    memcpy( g_host_cfg.szId, g_xDevInfoReg.xDevInfo.xPN.szPN, sizeof(DevPN_t) );
}

static void prvReadHostCfgFromFs( void )
{
    if( !board_cfg_read( HOST_CFG_OFS, &g_host_cfg, sizeof(g_host_cfg)) ) {
...
{
    if( !board_cfg_write( HOST_CFG_OFS, &g_host_cfg, sizeof(g_host_cfg)) ) {
...
void jsonHostCfg(int n, cJSON *pItem)
{
...
void setHostCfgWithJson(cJSON *pCfg)
{
    ...
    const char *id = cJSON_GetString( pCfg, "id", VAR_NULL );

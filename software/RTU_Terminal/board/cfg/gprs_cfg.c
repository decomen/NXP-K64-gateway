...
{
    if (!board_cfg_read(GPRS_NET_CFG_OFS, &g_gprs_net_cfg, sizeof(g_gprs_net_cfg))) {
...
{
    if (!board_cfg_write(GPRS_NET_CFG_OFS, &g_gprs_net_cfg, sizeof(g_gprs_net_cfg))) {
        ; //prvSetGPRSNetCfgDefault();
    }
}
...
{
    if (!board_cfg_read(GPRS_WORK_CFG_OFS, &g_gprs_work_cfg, sizeof(g_gprs_work_cfg))) {
...
{
    if (!board_cfg_write(GPRS_WORK_CFG_OFS, &g_gprs_work_cfg, sizeof(g_gprs_work_cfg))) {
        ; //prvSetGPRSWorkCfgDefault();
...
{
    static rt_uint32_t ulLastTime = 0;
    char tmp[128] = { 0 };

    if (ulLastTime == 0 || (ulLastTime % 10 == 0)) {
        bGPRS_GetCSQ(RT_FALSE);
        bGPRS_GetCREG(RT_FALSE);
        bGPRS_GetCOPS(RT_FALSE);
        bGPRS_GetSMOND(RT_FALSE);
    }

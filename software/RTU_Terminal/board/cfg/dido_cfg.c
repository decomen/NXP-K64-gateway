...
{
    if( !board_cfg_read( DI_CFG_OFS, &g_di_cfgs[0], sizeof(g_di_cfgs) ) ) {
        prvSetDiCfgDefault();
    }
}
...
{
    if( !board_cfg_write( DI_CFG_OFS, &g_di_cfgs[0], sizeof(g_di_cfgs) ) ) {
        ;//prvSetDiCfgDefault();
    }
}
...
{
    if( !board_cfg_read( DO_CFG_OFS, &g_do_cfgs[0], sizeof(g_do_cfgs) ) ) {
        prvSetDoCfgDefault();
    }
}
...
{
    if( !board_cfg_write( DO_CFG_OFS, &g_do_cfgs[0], sizeof(g_do_cfgs) ) ) {
        ;//prvSetDoCfgDefault();
    }
}

...
static struct rt_mutex s_zgb_std_mutex;
...
void zgb_check_online(void)
{
    ...
    while (node) {
        ...
        if (node->online) {
            ...
            // 移除下线节点
        } else if (nowtime - node->offtime >= (g_zigbee_cfg.ulLearnStep * 20)) {
            if (prev) {
                prev->next = next;
            } else {    // 头结点后移
                s_zgb_mnode = next;
...
static void json_zgb_mnode(zgb_mnode_t *mnode, cJSON *pItem)
{
    ...
    if (mnode) {
        ...
        if (mnode->uptime > 0) {
            localtime_r((time_t *)&mnode->uptime, &lt);
        ...
        memset(buf, 0, sizeof(buf));
        if (mnode->offtime > 0) {
            localtime_r((time_t *)&mnode->offtime, &lt);
            sprintf(buf, "%u/%02d/%02d %02d:%02d:%02d",
                    lt.tm_year + 1900,
                    lt.tm_mon + 1,
                    lt.tm_mday,
